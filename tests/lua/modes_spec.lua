...
describe("modes_spec", function()
	...
	it("modes.toggleMode updates active list - buffer", function()
		...
		-- assert mode was added
		assert.same(modes.activeModes["1"][testData.id], { id = testData.id, icon = testData.icon })
		modes.toggleMode(mode, options1)
		-- assert mode was removed
	...
	it("modes.getActiveModesIcons return valid list - buffer", function()
		...
		local mode2 = modes.createIfNotPresent(testData2.id, activationFn, deactivationFn, testData2.icon)
		local mode3 = modes.createIfNotPresent(testData3.id, activationFn, deactivationFn, testData3.icon)
		local mode4 = modes.createIfNotPresent(testData4.id, activationFn, deactivationFn, testData4.icon)
	...
	describe("modes mix validation", function()
		...
		before_each(function()
			...
			mode1 = modes.createIfNotPresent(testData1.id, activationFn, deactivationFn, testData1.icon)
			mode2 = modes.createIfNotPresent(testData2.id, activationFn, deactivationFn, testData2.icon)
		end)
